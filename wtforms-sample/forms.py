...
class UserInForm(Form):
    ...
    name = StringField('名前:',validators=[DataRequired('名前は必須入力です')],
                       render_kw={"placeholder": "（例）小瘪三"})
    # 年齢：整数値入力
    age = IntegerField("年齢：",validators=[NumberRange(18,100,
                        '入力範囲は18歳から100歳です')],default=20)
    #パスワード：パスワード入力
    password=PasswordField('パスワード：',validators=[Length(1,10,
                                    'パスワードの長さは１文字以上10文字以内です'),
